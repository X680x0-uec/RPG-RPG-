    ...
    void Update()
    {
        if (slot_flag == "true") //スロットの回転を止めるためのフラグが立っている場合
        {
            if (image_location[slot_error].anchoredPosition.y < -185.0f) //スロットのずれが真ん中より大きくなったら
            {
                ...
                Slot_Results results = FindFirstObjectByType<Slot_Results>();//シーン内からSlot_Resultsを取得
                if(results.AllStopped())//すべてのスロットが止まったら出た目の結果を集計する
                {
                    results.CountSlot(results.slot_reels);//CountSlot関数の呼び出し
                }
            }
        }
